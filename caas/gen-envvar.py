...
def generate_secret_key(key_length, display_key):
    key_array = [random.SystemRandom().choice(string.printable[:-6])
...
if __name__ == '__main__':
    ...
    print('---')
    print('The "cas" service uses PostgreSQL as the database backend. Each machine (dev or production) may have differing database names and credentials for this service. Please provide the following information to connect your local copy of this service to the PostgreSQL server you have set up for this service.')
    print('')
    ...
    if os.name == 'posix':  # Yeah, we're in UNIX boiz.
        ...
        print('  $ chmod +x services/user/core/set-envvars.sh')
        print('  $ (source | .) ./services/user/core/set-envvars.sh')
        print('')
